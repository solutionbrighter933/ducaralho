...
const stripe = new Stripe(stripeSecret || 'sk_test_default', {
  appInfo: {
...
Deno.serve(async (req) => {
  try {
    ...
    const { price_id, success_url, cancel_url, mode, trial_days } = await req.json();
...
function validateParameters<T extends Record<string, any>>(
  values: T & { trial_days?: number },
  expected: Expectations<T>
): string | undefined {
  for (const parameter in values) {
    const expectation = expected[parameter];
    if (!expectation) continue; // Skip parameters not in expectations
    ...
    if (expectation === 'string') {
      ...
      if (value && !expectation.values.includes(value)) {
